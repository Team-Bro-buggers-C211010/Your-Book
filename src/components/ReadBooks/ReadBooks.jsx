...
const ReadBooks = ({ sortBy }) => {
    ...
    useEffect(() => {
        ...
        if (allData.length > 0) {
            ...
            setReadBooks(sortedReadBookList);
        }
    }, [sortBy, allData]);
    ...
    return (
        <>
            {readBooks.map((read) => (
                <div key={read.bookId} className="card lg:card-side bg-base-100 shadow-xl border border-[#1313130D] p-6 gap-x-6 mb-6">
                        ...
                        <div className='flex flex-col md:flex-row fontWorkSans text-sm md:text-base'>
                            <div className='flex gap-x-3'>
                                ...
                                {read.tags.map((tag, index) => (
                                    <div key={index} className='bg-[#23BE0A0D] text-[#23BE0A] text-sm md:text-base px-4 py-2 mb-4 rounded-full flex fontWorkSans font-medium'>#{tag}</div>
                                ))}
                            </div>
